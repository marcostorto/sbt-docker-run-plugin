...
object DockerRunPlugin extends AutoPlugin {
  ...
  override lazy val projectSettings = Seq(
    dockerRunContainerName := s"${name.value}-it-docker-${version.value}",
    dockerRunEnvironment := Seq.empty,
    dockerRunStart := {
      val envParameters = dockerRunEnvironment.value.map {
      ...
      if (!waitHealthy(dockerRunContainerName.value, streams.value.log)) {
